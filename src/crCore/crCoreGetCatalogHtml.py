...
def _get_content_url_tag(catalog_node, doc, tag, text):
    ...
    if catalog_node.have_content_url:
        with tag("span", crCatalogNodeUrl=catalog_node.content_url, klass="catalog_node_url have_url", onclick="catalog_node_list_script_namespace.onclickCatalogNode(this)"):
            text(catalog_node.catalog_name)
    #
    # else if no content url:
    else:
        # A tag but without url
        with tag("span", klass="catalog_node_url no_url", onclick="catalog_node_list_script_namespace.onclickCatalogNode(this)"):
            text(catalog_node.catalog_name)
